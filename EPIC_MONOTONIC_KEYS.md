...
**Target:** `monotonic-order` 0.2–0.7
...
caller supplies or derives an order-preserving key, and MonotonicOrder returns a
stable permutation.
...
```c
int monotonic_order_argsort_u64(
    const uint64_t *keys,
    size_t n,
    size_t *indices,
    size_t *workspace
...
```c
int monotonic_order_lexargsort_u64(
...
generator; MonotonicOrder can radix-sort those generated bytes but must not invent
linguistic rules.
...
- add `monotonic_order_argsort_u64` C API;
...
6. existing `MonotonicOrder` model and `float64 sort` remain backward compatible;
